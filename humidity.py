...
def send_visualization_email(df):
    ...
    message = str(df[["humidity", "room_temp", "cpu_temp"]].corr()) + "\n\n"+ str(df[["humidity", "room_temp", "cpu_temp"]].describe()).format("utf8") + "\n\n" + str(df[["timestamp", "humidity", "room_temp", "cpu_temp"]].tail(6))
...
def collect_and_save_to_db():
    ...
    insert_measurements_into_db(connection=con,timestamp=timestamp, humidity=humidity, room_temp=room_temp, cpu_temp=cpu_temp)
    log.info("Done")
