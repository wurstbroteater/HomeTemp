...
class Command:
    ...
    def execute(self):
        ...
        print(f"Executing command {self.id} with parameters {self.params}")
...
class CommandService:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"Failed to connect to the mail server: {str(e)}")
            raise
    ...
    def _get_emails(self):
        try:
            ...
            print(f"Error occurred while fetching emails: {str(e)}")
    ...
    def _delete_email(self, email_id):
            try:
                ...
                print(f"Error occurred while deleting email: {str(e)}")
    ...
    def _get_and_delete_email_with_command(self, delete_mail=True):
        ...
        try:
            ...
            print(f"Error occurred while processing emails: {str(e)}")
        finally:
    ...
    def _parse_received_command(self, commander, header, body):
        ...
        if command_tokens is None:
            print(f"Commander '{commander}' passed invalid command '{header}'")
        elif len(command_tokens) == 0:
            print(f"Commander '{commander}' only passed valid prefix but no command")
        else:
            received_cmd_id = command_tokens[0]
            command = self._get_command_by_id(received_cmd_id)
            if command is None:
                print(f"Commander '{commander}' sent unknown command id '{received_cmd_id}'")
            else:
                print(f"Commander '{commander}' requested command id '{received_cmd_id}'")
        return command
    ...
    def get_received_command_requestes(self):
        commands = self._get_and_delete_email_with_command(delete_mail=False)
        valid_commands = []
...
if __name__ == "__main__":
    try:
        ...
        cmd_name = 'plot'
        cmd_params = []
        command_service.add_command_syntax((cmd_name, cmd_params, foo))
        print("go")
        valid_commands = command_service.get_received_command_requestes()
        for cmd in valid_commands:
            print(f"Received call to execute : {cmd}")
            cmd.function()
